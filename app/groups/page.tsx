...
function emptyRow(team: string): TeamRow {
  return { team, flag: FLAGS[team] ?? "🏳️", mp: 0, w: 0, d: 0, l: 0, gd: 0, pts: 0 };
}
...
function applyResult(row: TeamRow, gf: number, ga: number) {
  ...
  else if (gf === ga) { row.d += 1; row.pts += 1; }
  else { row.l += 1; }
}
...
export default function GroupsCM2026Page() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.from("matches").select("id, group_name, home_team, away_team, home_score, away_score, is_finished").eq("stage", "groups").order("order_index", { ascending: true });
    ...
    <main style={{ backgroundImage: "linear-gradient(rgba(7,19,39,0.75), rgba(7,19,39,0.92)), url('/images/pronosticuri.jpeg')", backgroundSize: "cover", backgroundPosition: "center" }}>
      <AppShell>
        <section className="card p-6 md:p-8">
          ...
          <p className="mt-2 text-white/70">Clasamente și scoruri actualizate din Admin.</p>
        </section>

        {loading ? <div className="mt-6 card p-6">Se încarcă...</div> : (
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {letters.map((group) => (
              <div key={group} className="rounded-[28px] bg-white text-slate-900 p-5 shadow-sm">
                <div className="mb-4 flex items-start justify-between">
                  <div>
                    <div className="text-sm text-slate-500">Table</div>
                    <div className="text-base text-slate-600">World Cup</div>
                    <div className="text-lg font-semibold">Group {group}</div>
                  </div>
                  <div className="text-xl text-slate-400">›</div>
                </div>

                <div className="grid grid-cols-[26px_1.7fr_repeat(6,36px)] items-center gap-2 border-b border-slate-200 pb-2 text-xs font-semibold text-slate-600">
                ...
                <div>
                  {grouped.standings[group].map((row, index) => (
                    <div key={row.team} className="grid grid-cols-[26px_1.7fr_repeat(6,36px)] items-center gap-2 border-b border-slate-100 py-3 last:border-b-0">
                      <div className="text-sm font-medium">{index + 1}</div>
                      <div className="flex items-center gap-3 min-w-0"><span className="text-lg">{row.flag}</span><span className="truncate text-[15px] font-medium">{row.team}</span></div>
                ...
                <div className="mt-4 space-y-2 rounded-2xl bg-slate-100 p-3 text-sm">
