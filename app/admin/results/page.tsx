...
export default function AdminResultsPage() {
  ...
  useEffect(() => {
    async function checkAuth() {
      ...
      const { data: prof } = await supabase.from("profiles").select("id, is_admin").eq("id", user.id).single();
  ...
  const fetchData = useCallback(async () => {
    ...
    if (predRes.data && profilesRes.data) {
      ...
      const predsWithProfiles = predRes.data.map(
        (p: { user_id: string; match_id: string; predicted_home: number; predicted_away: number }) => ({
          ...p,
        ...
        <div className="flex gap-2 border-b border-white/10 pb-0">
          <button onClick={() => setActiveTab("results")}
            className={`rounded-t-lg px-5 py-2.5 text-sm font-medium transition-all ${
              activeTab === "results" ? "border border-white/10 border-b-gray-950 bg-white/10 text-white" : "text-white/50 hover:text-white/80"
            }`}>
            📋 Rezultate meciuri
          </button>
          <button onClick={() => setActiveTab("users")}
            className={`rounded-t-lg px-5 py-2.5 text-sm font-medium transition-all ${
              activeTab === "users" ? "border border-white/10 border-b-gray-950 bg-white/10 text-white" : "text-white/50 hover:text-white/80"
            }`}>
            👥 Pronosticuri useri
          </button>
        </div>
            ...
            <div className="space-y-3 rounded-xl border border-white/10 bg-white/5 p-4">
              ...
              {filterStage === "groups" && (
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="mx-1 self-stretch w-px bg-white/10" />
                  {["all", "1", "2", "3"].map(md => (
